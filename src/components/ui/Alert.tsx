...
const Alert = ({ title, message, type }: { title: string, message: string, type: 'info' | 'warning' | 'error' }) => {
    ...
    return (
        <div role="alert" className={`rounded border-s-4 p-4 w-full md:w-[500px] ${variant({box: type})}`}>
            <div className={`flex items-center gap-2 ${variant({header: type})}`}>
                { icon }
                <strong className="block font-medium"> { title } </strong>
            </div>

            <p className={`mt-2 text-sm ${variant({content: type})}`}>
                { message }
            </p>
        </div>
    )
